...
const MobileUserContent = () => {
  ...
  const { data: posts } = useQueryGetAllPosts();
  const { data: todos } = useQueryGetAllTodos();
            ...
            userList.map((user, index) => (
              <Fragment key={user.id}>
                <UserCard
                  index={index}
                  user={user}
                  onClick={(user) => handleRowClick(user as GetUser)}
                />
              </Fragment>
            ))
          ) : (
...
export default MobileUserContent;
